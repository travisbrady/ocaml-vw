let stubgen prefix c_headers bindings =
  let ml_out = open_out ("lib/" ^ prefix ^ "_generated.ml")
  and c_out = open_out ("lib/" ^ prefix ^ "_stubs.c") in
...
let () = stubgen "vw" "#include <stddef.h>\n#include <vowpalwabbit/vwdll.h>" (module Vw_bindings.C)
